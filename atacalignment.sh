...
prefix=$(cat $filename | awk -F '_' '{print $1"_"$2"_"$3"_"$4}' | sort | uniq | head -n $SGE_TASK_ID | tail -n 1)

MYOUTDIR="Alignment"
mkdir -p ${MYOUTDIR}

bwa mem -t ${NSLOTS} -M ${ref} ${prefix}_R1.fq.gz ${prefix}_R2.fq.gz | samtools view -bS - > ${MYOUTDIR}/${prefix}.bam

samtools sort ${MYOUTDIR}/${prefix}.bam -o ${MYOUTDIR}/${prefix}.sort.bam

# need RGPU=flowcell name; find by zcat file | head -n 1, file name including.fq.gz
